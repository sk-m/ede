...
window.onload = function() {
    ...
    // User notificaions
    ede.element.user_notifications_root_el = document.getElementById("ede-usernotifications-root");

    // If user is logged in (if the user is not logged in, this element will not be available)
    if(ede.element.user_notifications_root_el) {
        // User is logged in, get the notifications button
        ede.element.user_notifications_button_el = document.getElementById("ede-usernotifications-button");

        // Get the notifications list element
        ede.element.user_notifications_list_el = ede.element.user_notifications_root_el.getElementsByClassName("notifications-list")[0];

        // Get user's notifications status
        ede.apiCall("user/get_notifications_status", {}, false)
        .then((response) => {
            const client_has_unread_notifications = response["user/get_notifications_status"].has_unread_notifications;

            // If user has unread notifications, highlight the notifications button
            if(client_has_unread_notifications) {
                ede.element.user_notifications_button_el.classList.add("highlighted");
            }
        }).catch(error => {
            ede.showAPIErrorNotification("user-getnotificationsstatus", error);
        })
    }
...
window.ede = {
     ...
     */
    apiCall: function(action, params, is_post = false, send_empty_strings = false) {
        // TODO @cleanup

        return new Promise((resolve, reject) => {
            let request_body = "";

            // Convert parameters to a string (?a=1&b=2)
            if(params) {
                for(const i in params) {
                    // Do not send empty string params
                    if(!send_empty_strings &&
                        (params[i] === null || params[i] === undefined || params[i] === "")) continue;
    ...
    /**
     * Show a notification about an apiCall error
     *
     * @param {string} action_name Internal action name that will be used in a notificaion id
     * @param {any} error_object Error object, recieved from a apiCall rejection
     */
    showAPIErrorNotification: function(action_name, error_object) {
        if(!action_name || !error_object) throw new Error("Invalid showAPIErrorNotification call");

        ede.showNotification(`${ action_name }-error`,
            "Error", error_object.message || `<code>${ error_object.error_type }</code>.` || "Unknown error occured", "error");
    },
    ...
    logoutUser: function(skip_popup) {
        const logout = () => {
            ...
            .catch(error => {
                ...
                ede.showAPIErrorNotification("user-logout", error);
            })
        };
...
<div class="ui-input-box" style="margin-top: 25px">
    ...
    toggleToEditor: function() {
        ...
        if(!ede.current_page.editor_loaded) {
            ...
            .then(editor_response => {
                ...
                // Insert the HTML
                ede.element.template.ede_page_editor_root.innerHTML = editor_response.get_editor_html.html;
                ...
                save_form.submit.onclick = () => {
                    ...
                    .catch(save_error => {
                        ede.showAPIErrorNotification("savepage", save_error);
                    })
                }
            })
            .catch(editor_error => {
                ede.showAPIErrorNotification("geteditorhtml", editor_error);
            });
        }
    },
    ...
    toggleToRevisions: function() {
        ...
        if(!ede.current_page.revisions_loaded) {
            ...
            .then(revisions_response => {
                const revisions = revisions_response["revision/get"].revisions;

                ede.current_page.revisions_loaded = true;
                ede.current_page.revisions = revisions;
                ...
                const show_diff = (revid_from, revid_to) => {
                    ...
                    .then(diff_response => {
                        ...
                        ede.element.template.ede_page_revisions_diff.innerHTML = diff_response["revision/diff"].diff;
...
<div class="ui-text margin-bottom">
    ...
    <div class="ui-revision-item">
        ...
        <span class="summary">(${ to_obj.summary })</span>
                    ...
                    .catch(diff_error => {
                        ede.showAPIErrorNotification("getrevisiondiff", diff_error);
                    })
                }

                const revisions_fragment = ede.tools.constructRevisionsHTML(revisions, true, {
                    prev: {
            ...
            .catch(revisions_error => {
                ede.showAPIErrorNotification("getpagerevisions", revisions_error);
            });
        }
    },
    ...
    navigate: function(new_url, replace = false) {
        ...
        .then(page_response => {
            ...
            ede.updatePage(page_response["page/get"].page);
        });
     ...
     */
    refresh: function() {
        ede.navigate(location.pathname + location.search);
    },
    ...
    updateForms: function(reference_el) {
        ...
        for(const form of forms) {
            ...
            if(form.hasAttribute("categories-editor")) {
                // TODO @performance
                const expand_btn = form.getElementsByClassName("expand-btn")[0];
                const expand_btn_icon = expand_btn.getElementsByTagName("i")[0];

                const save_btn = form.getElementsByClassName("save-btn")[0];

                const cats_list = form.getElementsByClassName("list")[0];
                const add_container = form.getElementsByClassName("add-container")[0];

                const input_el = form.getElementsByTagName("input")[0];
                const selector_box_el = form.getElementsByClassName("selector-box")[0];

                // Show/hide "add category" container
                expand_btn.onclick = () => {
                    if(add_container.hasAttribute("expanded")) {
                        add_container.removeAttribute("expanded");
                        expand_btn_icon.className = "fas fa-plus";
                    } else {
                        add_container.setAttribute("expanded", "");
                        expand_btn_icon.className = "fas fa-chevron-left";

                        input_el.focus();
                    }
                }

                // On "add category" name input focus (show category selector box)
                input_el.onfocus = () => {
                    selector_box_el.hidden = false;
                }

                // On "add category" name input blur (hide category selector box)
                input_el.onblur = () => {
                    selector_box_el.hidden = true;
                }
                ...
                form.onsubmit = () => {
                    ...
                    new_cat.className = "item new";
                    new_cat.innerText = input_el.value;
                ...
                save_btn.onclick = () => {
                    ...
                    save_btn.getElementsByTagName("i")[0].className = "fas fa-ellipsis-h";
    ...
    /**
     * Update the systempage side menu state on page load or update (shrunk or normal size)
     *
     * We set the state to the last state any sidemenu has been in. So, if user shrunk the last sidemenu, all sidemenus will be shrunk
     * by default. Just like with a main menu.
     */
    updateSystempageSidemenuState: function() {
        if(localStorage.getItem("ede_ui_systempage_sidemenu_hidden") === "true") {
            const sidemenu_root = document.querySelector("#ede-page-content .ui-systempage-sidebar-right");
            if(sidemenu_root) sidemenu_root.setAttribute("shrunk", "");
        }
    },

    /**
     * Toggle the systempage side menu (shrunk or normal size)
     */
    toggleSystempageSidemenuState: function() {
        const sidemenu_root = document.querySelector("#ede-page-content .ui-systempage-sidebar-right");

        if(sidemenu_root.hasAttribute("shrunk")) {
            sidemenu_root.removeAttribute("shrunk");
            localStorage.setItem("ede_ui_systempage_sidemenu_hidden", "false");
        } else {
            sidemenu_root.setAttribute("shrunk", "");
            localStorage.setItem("ede_ui_systempage_sidemenu_hidden", "true");
        }
    },
    ...
    updateElements: function() {
        ...
        // System namespace related
        // TODO @placeholder namespaces should not be hardcoded
        if(ede.current_page.address.namespace === "System" || ede.current_page.address.namespace === "User") {
            ede.updateSystempageSidemenuState();
        }
    ...
    tools: {
        systempage: {
            setBreadcrumb: function(id, text, classname, href) {
                // Try getting the DOM element of the target breadcrumb
                const breadcrumb_el = document.getElementById("systempage-breadcrumbs-" + id);

                if(breadcrumb_el) {
                    // Such breadcrumb exists. Just update it
                    breadcrumb_el.innerText = text;
                    if(classname) breadcrumb_el.className = "item " + classname;
                    if(href) {
                        breadcrumb_el.href = href;
                        breadcrumb_el.title = `Go to ${ href }`;
                    }
                } else {
                    // Such breadcrumb does not exist. Create it
                    const container_el = document.getElementById("systempage-breadcrumbs-container");

                    container_el.innerHTML += `
<div class="separator"><i class="fas fa-chevron-right"></i></div>\
<a ${ href ? `href="${ href }" title="Go to ${ href }"` : "" }class="item" id="systempage-breadcrumbs-${ id }">\
...
${ text }</a>`;
        ...
        /**
         * Construct a "7 minues ago"-type string from a timestamp
         *
...
`;
     ...
     * @param {string} buttons_html buttons html (set to false to remove the buttons container completely)
    ...
    showPopup: function(id, title, body_html, buttons_html, button_actions, width = 440) {
        // Default buttons
        if(buttons_html === undefined) buttons_html = `\
<div class="left"></div>
<div class="right">
...
<div class="middle">
    ...
    element: {
        ...
        user_notifications_root_el: null,
        user_notifications_root_shown: false,
        user_notifications_button_el: null,
        user_notifications_list_el: null,
        user_notifications_focus_target_el: null,
    ...
    user_notifications: {
        ...
        /**
         * Show or hide a notifications panel
         */
        toggle: function() {
            if(ede.element.user_notifications_root_shown) {
                // Panel is open - close
                ede.element.user_notifications_root_el.removeAttribute("active");
                ede.element.user_notifications_root_shown = false;

                ede.element.user_notifications_button_el.classList.add("inactive");
            } else {
                // Panel is closed - open

                // Get notifications from the server, if not yet loaded
                if(!ede.user_notifications.loaded) {
                    ede.user_notifications.loaded = true;
                    ede.user_notifications.get_notifications();
                }

                ede.element.user_notifications_root_el.setAttribute("active", "");
                ede.element.user_notifications_root_shown = true;

                ede.element.user_notifications_button_el.classList.remove("inactive");
            }
        },

        /**
         * Focus on a notification
         *
         * @param {DOMElement} notification_el Notification DOM element
         */
        toggle_focus: function(notification_el) {
            if(notification_el.hasAttribute("focus-target")) {
                // Remove the focus, if this notification is already focused

                notification_el.removeAttribute("focus-target");
                ede.element.user_notifications_list_el.removeAttribute("focused");

                ede.element.user_notifications_focus_target_el = null;
            } else {
                // Focus on a notification

                ede.element.user_notifications_list_el.setAttribute("focused", "");

                notification_el.setAttribute("focus-target", "");
                ede.element.user_notifications_focus_target_el = notification_el;

                // Decrease the number of unread notifications, if the notification we are currently focusing on is unread
                if(notification_el.hasAttribute("unread")) {
                    notification_el.removeAttribute("unread");
                    notification_el.setAttribute("read", "");

                    // Mark as read on the server
                    ede.apiCall("user/notification_mark_read", {
                        notification_id: notification_el.getAttribute("notification-id")
                    }, true);

                    // Update the number of unread notifications
                    ede.user_notifications.unread_num--;

                    if(ede.user_notifications.unread_num === 0) {
                        // No unread notifications left, remove the unread notifications bubble and disable notifications button's
                        // highlighted state

                        ede.element.user_notifications_root_el.getElementsByClassName("unread-bubble")[0].classList.add("hidden");
                        ede.element.user_notifications_button_el.classList.remove("highlighted");
                    } else {
                        // Decrease the number of unread notifications

                        ede.element.user_notifications_root_el.querySelector(".unread-bubble > span").innerText = ede.user_notifications.unread_num;
                    }
                }
            }
        },
        ...
        /**
         * Get notifications from the server and populate the notifications panel list
         */
        get_notifications: function() {
            ede.apiCall("user/get_notifications", {
                from: (ede.user_notifications.last_id || -1) + 1,
                records_number: 50
            }, false)
            .then((response) => {
                const notifications_list = response["user/get_notifications"].notifications;

                // No notifications returned
                if(notifications_list.length === 0) return;

                // Create a fragment for notifications list
                const notifications_fragment = document.createDocumentFragment();

                // Set last id (first array item, as they are ordered from newest to oldest)
                ede.user_notifications.last_id = notifications_list[0].id;

                let unread_num = 0;

                for(const notification_data of notifications_list) {
                    if(!notification_data.is_read) unread_num++;

                    // Create a notification
                    const notification_el = document.createElement("div");
                    notification_el.setAttribute("notification-id", notification_data.id);
                    notification_el.className = "notification";

                    // Toggle on click
                    notification_el.addEventListener("click", () => {
                        ede.user_notifications.toggle_focus(notification_el);
                    });

                    if(!notification_data.is_read) {
                        notification_el.setAttribute("unread", "");
                    }

                    // Create action buttons
                    const actions_container_el = document.createElement("div");
                    actions_container_el.className = "actions-container";

                    // Add actions
                    for(const action_data of notification_data.actions) {
                        const action_button_el = document.createElement("a");
                        action_button_el.className = `action-button ${ action_data.type }`;
                        action_button_el.innerText = action_data.text;

                        // On action lick
                        action_button_el.onclick = () => {
                            // Navigate to the link (either a static link or a dynamic link)
                            ede.navigate(action_data.href || notification_data.additional_info[action_data.dynamic_href_key]);

                            // Close the notifications panel
                            ede.user_notifications.toggle();
                        };

                        // Append the action
                        actions_container_el.appendChild(action_button_el);
                    }

                    // Set the main html for the notification
                    // TODO @performance ?
                    notification_el.innerHTML = `\
<div class="left">
    <div class="icon"><i class="${ notification_data.icon_class }"></i></div>
</div>
<div class="right">
    <div class="title">
        <div class="time">
            <span title="${ new Date(notification_data.timestamp * 1000).toLocaleString() }">${ ede.tools.formatTimeString(notification_data.timestamp) }</span>
            ${ notification_data.is_read ? "" : "<div class=\"unread-circle\"></div>" }
        </div>

        <span>${ notification_data.title }</span>
    </div>
    <div class="full-text">${ notification_data.text }</div>
    <div class="additional-text">${ notification_data.additional_text || "" }</div>
</div>`;

                    notification_el.append(actions_container_el);
                    notifications_fragment.appendChild(notification_el);
                }

                // Update unread notifications number
                if(unread_num === 0) {
                    // No unread notifications left, we can just remove the unread nofifications bubble

                    ede.element.user_notifications_root_el.getElementsByClassName("unread-bubble")[0].classList.add("hidden");
                } else {
                    ede.element.user_notifications_root_el.querySelector(".unread-bubble > span").innerText = unread_num;
                }

                // Save the number of unread notifications
                ede.user_notifications.unread_num = unread_num;

                // Append the notifications
                ede.element.user_notifications_list_el.appendChild(notifications_fragment);
            })
            .catch(error => {
                ede.showAPIErrorNotification("getnotifications", error);
            })
        }
    },
