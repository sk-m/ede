        ...
        !ede.element.template.ede_page_main_content_container.className.includes("nocontainer")) {
            ...
            if(ede.element.page_action_buttons.manage) {
                ...
                ede.element.page_action_buttons.manage.onclick = () => {
                    ede.navigate(`/System:WikiPageManagement/info?title=${ ede.current_page.address.namespace }:${ ede.current_page.address.name }`);
                };
                ...
                ede.element.page_action_buttons.manage.oncontextmenu = () => {
                    ...
                    for(const link_el of links) {
                        link_el.onclick = e => {
                            ...
                            ede.navigate(`/System:WikiPageManagement/${ link_el.getAttributeNames()[1] }?title=${ ede.current_page.address.namespace }:${ ede.current_page.address.name }`);
                        };
                    }
