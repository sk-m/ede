...
function _ede_prism_onload() {
    if(ede.current_page && ede.current_page.page_lang !== "none") {
        ...
        const code_beautified = `<pre><code class="language-${ lang }">${
            Prism.highlight(decodeURIComponent(ede.current_page.parsed_content), prism_lang, lang)
        }</code></pre>`;
...
window.ede = {
     ...
     */
    navigate: function(new_url) {
        history.replaceState({}, undefined, new_url);
    ...
    updateForms: function() {
        ...
        for(const form of forms) {
            // Get all inputs
            // TODO we should search by "input" attrubute instead (<div input></div>)
            const all_inputs = form.querySelectorAll(["input[data-handler]", "button", ".ui-checkbox-1", ".ui-input-dropdown1"]);
