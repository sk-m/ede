...
window.ede = {
    ...
    toggleToEditor: function() {
        ...
        // Buttons
        const edit_button = ede.element.template.ede_page_actionbuttons.querySelector(".page-edit");
        const read_button = ede.element.template.ede_page_actionbuttons.querySelector(".page-read");
        ...
        if(!ede.current_page.editor_loaded) {
            ede.apiCall("get_editor_html", {
                page_namespace: ede.current_page.address.namespace,
                page_name: ede.current_page.address.name
            }, false)
    ...
    navigate: function(new_url, replace = false) {
        ...
        ede.apiCall("page/get", { title: new_url.substr(1) })
        .then(page => {
            ede.updatePage(page);
        });
    ...
    updatePage: function(new_page) {
        ...
        // Main templates
        ede.setTemplate("ede_page_title", decodeURIComponent(new_page.display_title));
        ...
        // Update title
        document.title = `${ new_page.display_title } – ${ ede.instance_display_name }`;
            ...
            for(const el of all_inputs) {
                ...
                if(el.nodeName === "INPUT") {
                    ...
                    el.addEventListener("click", () => {
                        el.parentElement.closest("div[input-container]").getElementsByClassName("popup")[0].classList.remove("shown");
                    });
    ...
    updateElements: function() {
        ...
        // Page edit link
        if(ede.element.template.ede_page_actionbuttons) {
            // TODO ctrl+F these two lines. We query them too much
            const edit_button = ede.element.template.ede_page_actionbuttons.querySelector(".page-edit");
            const read_button = ede.element.template.ede_page_actionbuttons.querySelector(".page-read");

            // Manage button
            ede.element.template.ede_page_actionbuttons.querySelector(".page-manage").onclick = () => {
                const full_address = `${ ede.current_page.address.namespace }:${ ede.current_page.address.name }`;

                ede.navigate(`/System:WikiPageManagement/info/${ full_address }`);
            };

            if(read_button && edit_button) {
                edit_button.onclick = ede.toggleToEditor;
                read_button.onclick = ede.toggleToContent;
            }
        }
    ...
    tools: {
        constructRevisionsHTML: function(revisions) {
            let final_string = "";
            ...
            for(const revision of revisions) {
                ...
                final_string += `
<div class="ui-revision-item" data-revid="${ revision.id }">
    <div data-revid="${ revision.id }" class="ui-checkbox-1 small">
...
`;
        ...
        if(button_actions) {
            for(const button_name in button_actions) {
                ...
                button_el.addEventListener("click", button_actions[button_name], false);
            }
        }
    },
