...
window.ede = {
    ...
    /**
     * Collapse a block (close / hide)
     *
     * @param {string} id DOM id of a block that is to be collapsed
     * @param {*} el DOM element of the button that collapsed the menu
     */
    // TODO maybe move into the skin script?
    collapse_block: function(id, button_el) {
        const el = document.getElementById(id).parentElement;

        if(el.className.includes("collapsed")) {
            el.classList.remove("collapsed");
            button_el.querySelector("i").style.transform = "rotate(0deg)";
        } else {
            el.classList.add("collapsed");
            button_el.querySelector("i").style.transform = "rotate(180deg)";
        }
    },
