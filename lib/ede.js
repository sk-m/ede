...
window.ede = {
    ...
    toggleToRevisions: function() {
        ...
        if(!ede.current_page.revisions_loaded) {
            ede.apiCall("revision/get", { pageid: ede.current_page.pageid }, false)
            .then(response => {
                ...
                // TODO @performance?
                ede.element.template.ede_page_revisions_root.innerHTML = "";
                ede.element.template.ede_page_revisions_root.appendChild(revisions_fragment);
            })
            .catch(error => {
    ...
    tools: {
        constructRevisionsHTML: function(revisions, with_checkboxes, buttons) {
            ...
            for(const revision of revisions) {
                ...
                rev_el.dataset.revid = revision.id;

                rev_el.innerHTML += `\
${ with_checkboxes ? `<div data-revid="${ revision.id }" class="ui-checkbox-1 small">
...
`;
                ...
                if(buttons) {
                    ...
                    for(const button_name in buttons) {
                        const butto_el = document.createElement("span");

                        butto_el.className = "button";
                        butto_el.innerHTML = `<i class="${ buttons[button_name].icon }"></i>`;
                        if(buttons[button_name].title) butto_el.title = buttons[button_name].title;

                        butto_el.onclick = (e) => { buttons[button_name].onclick(e, revision) };

                        buttons_el.appendChild(butto_el);
                    }
                }
