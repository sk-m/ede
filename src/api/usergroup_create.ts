...
export async function createUserGroupRoute(req: any, res: any, client_user?: User.User): Promise<void> {
    ...
    // Check if client has the rights to modify (in this case, create) user groups
    ...
    if(client_permissions_error) {
        apiSendError(res, new Rejection(RejectionType.GENERAL_ACCESS_DENIED, "Only users with 'modifyusergroups' right can create user groups"));
        return;
    }

    // Check if the group already exsists
    if(registry_usergroups.get()[req.body.new_group_name]) {
        apiSendError(res, new Rejection(RejectionType.GENERAL_INVALID_DATA, "A group with such name already exists"));
        return;
    }
