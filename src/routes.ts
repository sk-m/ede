...
export function pageTitleParser(raw_title: string, default_namespace: string = "Main"): Page.PageAddress {
    let name: string;
    ...
    // Decode the whole title
    raw_title = decodeURIComponent(raw_title);
    ...
    // Display name should not be uri encoded
    const display_name = he.encode(name);
    const display_namespace = he.encode(namespace);

    // Name should always be uri encoded
...
export function directRouteWrapper(page: Page.ResponsePage, additional_options?: any): string {
    ...
    // Sanitize frontend object

    // TODO we should have an object on frontend with common config items such as instance_name, current_skin, etc.
    // TODO!!! @hack @perfoemance what we do here with json.parse and decodeURI is a HORRIBLE hack
    const page_info_js: string[] =
    [`/* This script was automatically generated by the EDE engine. */\n\n\
ede.current_page = JSON.parse(decodeURI("${ encodeURI(JSON.stringify(frontend_page_object)) }"));
ede.instance_display_name = \`${ current_instance_displayname }\`;`];

    return Sqrl.Render(current_skin.html, {
        ede_page_title: page.address.display_title || "",
